...
select country, cast(round(avg(power(bore,3)*0.5),2) as numeric(10,2)) as weight from total
group by country
...
from (select maker, model, c
from Product
join (select count(model) c, maker m
from Product
group by maker ) b1
on maker = m) t1
join
...
-- 53. Determine the names of all ships in the Ships table that can be a Japanese battleship having at least nine main guns with 
